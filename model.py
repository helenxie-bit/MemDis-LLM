...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, past_key_value=None, kv_method=None):
        ...
        updated_key_value = (k, v) # update the KV cache for the next iteration
        ...
        y = att @ v # (B, nh, T, total_T) x (B, nh, total_T, hs) -> (B, nh, T, hs)
        y = y.transpose(1, 2).contiguous().view(B, T, C) # re-assemble all head outputs side by side

        # output projection
        y = self.resid_dropout(self.c_proj(y))

        return y, updated_key_value
...
class GPT(nn.Module):
    ...
    def generate(
        ...
        for token_gen_iter in range(max_new_tokens):
            ...
            total_idx = torch.cat((total_idx, idx_next), dim=1)
        ...
        if len(times) > 1:
            time_between_tokens = (sum(times) - time_to_first_token) / (batch_size * (len(times) - 1))
        else:
            time_between_tokens = 0
        tokens_per_second = batch_size * (seq_len + len(times)) / sum(times)
        ...
        metrics = {
            ...
            "time_between_tokens (s)": time_between_tokens,
